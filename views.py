...
def send_msg(request):
    ...
    try:
        ...
        # ==================== Token 控制与摘要逻辑 ====================
        MAX_TOKENS = 4096
        RESERVED_TOKENS = 500  # 为输出和当前输入预留空间
        MAX_CONTEXT_TOKENS = MAX_TOKENS - RESERVED_TOKENS

        # 简单 token 估算：1 token ≈ 4 个英文字符 或 1.5 个中文字符（保守估计）
        def estimate_tokens(text):
            # 中文字符多时更准确
            return max(len(text) // 4 + text.count(' ') // 2, len(text) // 1.5)

        # 获取所有历史消息（按时间顺序）
        all_messages = list(session.messages.order_by('created_at'))
        system_prompt = "你是一个专业的运维开发助手 AiOps AI。"
        context_msgs = [{"role": "system", "content": system_prompt}]
        current_tokens = estimate_tokens(system_prompt)

        # =============：知识库检索 =============
        knowledge_context = ""
        # knowledge_results = search_knowledge(user_input, top_k=2)
        # if knowledge_results:
        #     knowledge_context = "【参考知识库】\n" + "\n\n".join(knowledge_results)
        #     # 将知识库内容作为 system 消息插入（优先级高）
        #     context_msgs.append({"role": "system", "content": knowledge_context})
        #     current_tokens += estimate_tokens(knowledge_context)

        # 先加入用户新消息（但注意：它尚未保存到 history 查询集中，所以手动加到最后）
        # 实际上，all_messages 已包含刚创建的用户消息（因为 save 后 DB 已有）
        # 所以我们直接使用 all_messages

        # 从后往前尝试加入，但需要保持顺序 → 我们从前往后筛选，但优先保留最近的
        # 更合理做法：从最新往最旧取，直到超限
        recent_messages = []
        ...
        # 如果 recent_messages 未包含全部历史，说明需要摘要
        if len(recent_messages) < len(all_messages):
            # 需要摘要的旧消息
            old_messages = all_messages[:-len(recent_messages)] if recent_messages else all_messages
            ...
            if old_text.strip():
                ...
                summary_prompt = (
                    "请将以下对话历史压缩成一段简洁的上下文摘要，保留关键信息，以便后续对话理解。"
                    "只返回摘要内容，不要任何解释或前缀：\n\n" + old_text
                )
                summary_tokens = estimate_tokens(summary_prompt)
                if summary_tokens > MAX_CONTEXT_TOKENS - 100:
                    # 摘要本身太大，截断
                    summary_prompt = summary_prompt[: (MAX_CONTEXT_TOKENS - 100) * 4]
                ...
                try:
                    ...
                    summary = "（之前的对话历史摘要不可用）"

                # 插入摘要作为第一条消息（在 system 之后）
                context_msgs.append({"role": "system", "content": f"[对话历史摘要]: {summary}"})
                current_tokens = estimate_tokens(system_prompt) + estimate_tokens(summary) + 20
...
def search_knowledge(query: str, top_k: int = 3):
    """
    简单关键词匹配示例（实际可用向量搜索如 FAISS / Milvus / PGVector）
    返回最相关的 top_k 条知识条目内容
    """
    # 示例：模糊匹配（生产环境建议用语义搜索）
    results = KnowledgeBase.objects.extra(
...
class KnowledgeBase(models.Model):
    ...
    # 可选：embedding = VectorField(dim=768)  # 如果使用向量搜索